...
AGE_PUBKEY=$(grep -oE 'age1[a-z0-9]+' "$HOME/.config/sops/age/keys.txt" | head -1)
DATE_ISO=$(date -u +%Y-%m-%d)

# Sanity checks
[ -d "$TEMPLATE" ] || { log ERROR "Template not found at $TEMPLATE"; exit 1; }
[ -n "$AGE_PUBKEY" ] || { log ERROR "Could not extract age public key from ~/.config/sops/age/keys.txt"; exit 1; }
...
gh repo create "$GH_OWNER/$SLUG" --private --description "$TITLE" --confirm
...
find "$WORK" -type f \
  ...
  -exec sed -i \
    ...
    -e "s|{{MISSION_TITLE}}|$TITLE|g" \
    -e "s|{{DATE_ISO}}|$DATE_ISO|g" \
    -e "s|{{AGE_PUBKEY}}|$AGE_PUBKEY|g" \
    {} \;
